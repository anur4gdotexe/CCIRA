...
## What's Not Fully Wired Yet

A few things are in the code but not fully connected:

- **Route guards** (`ProtectedRoute`, `AdminRoute`) are commented out in `App.jsx` — routes are currently open without login
- **Register endpoint** (`POST /auth/register`) is called from the frontend but the backend controller for it isn't visible in the current code — may need to be added or verified
- **`MyComplaint.jsx`** exists as a page file but isn't in the router yet
- The AI engine is called from the frontend directly (not proxied through the backend), so both services need to be running for complaint submission to work
- Polygon coverage is currently only set up for **Noida** — other cities would need their GeoJSON files added
